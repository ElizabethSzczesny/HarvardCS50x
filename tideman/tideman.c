...
void recursive(int former_loser, int first_winner, int first_loser)
{
    ...
    for (int m = 0; m < candidate_count; m++)
    {
        if (locked[former_loser][m] == true && visited[m] == 0)
        {
            visited[m] = 1;
            if (locked[m][first_winner] == true)
            {
            ...
            {
            recursive(m, first_winner, first_loser);
                if (locked[first_winner][first_loser] == false)
                {
                    return;
                }
            }
...
void lock_pairs(void)
{
    ...
    while (n < pair_count)
    {
        ...
        printf("%i,%i\n", pairs[n].winner, pairs[n].loser);
        recursive(first_loser, first_winner, first_loser);
        n++;
    }
